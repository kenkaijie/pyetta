...
class Parser(ABC):
    ...
    def done(self) -> bool:
        ...
        :returns: True is parser is done (parser errors are communicated via the test suites).
        """

    @property
    @abstractmethod
    def test_suites(self) -> Optional[List[TestSuite]]:
        """The parsed test suites up to this point. Note this may be incomplete if called before the parser is stopped.
    ...
    def _add_parser_error(self, message: Optional[str] = None) -> None:
        ...
        if Parser.RESERVED_TEST_SUITE not in self._test_suites:
            # we only make it on the first use, or else we will have empty test suites which may trigger
            # failures (if fail on empty is set to true
            self._test_suites[Parser.RESERVED_TEST_SUITE] = TestSuite(name=Parser.RESERVED_TEST_SUITE)
        self._test_suites[Parser.RESERVED_TEST_SUITE].test_cases.append(test_case)
...
class UnityParser(Parser):
    ...
    def feed_data(self, data_chunk: bytes) -> None:
        try:
            ...
            if UnityParser.REGEX_TEST.match(line):
                ...
                self._test_suites[self._default_test_suite_name].test_cases.append(test_case)
    ...
    def stop(self, forced: bool = False) -> None:
        if self._state != UnityParser.ParserState.DONE:
            self._transition_state(UnityParser.ParserState.DONE)
            if forced:
                self._add_parser_error("Parser stopped before unit test output stopped.")

    def _transition_state(self, new_state: ParserState) -> None:
        if new_state != self._state:
            log.debug(f"Transitioning Unity parser from {self._state} -> {new_state}")
